...
- `src/lib/api/server-client.ts` is the server-only `portfolio-api` client. It forwards request cookies when needed, defaults authenticated requests to `no-store`, and normalizes backend errors.
- `src/lib/api/browser-client.ts` is the browser client. It calls the same-origin `/api` BFF and coordinates access-token refresh without exposing authentication cookies to JavaScript.
- `src/lib/api/errors.ts` owns shared backend problem parsing. Existing interactive dashboard reads use the compatibility adapter in `src/lib/utils/api`, which delegates to the browser client.
...
Feature mutations cross a Server Action boundary. Projects uses feature-local `project.actions.ts` and `project.service.ts` modules; the remaining Redux mutation thunks call the allowlisted `dashboardMutation` adapter, which delegates to `src/lib/server-actions/dashboard-mutations.actions.ts`. That action performs endpoint-specific Zod validation, requires authentication for owner operations, returns serializable `ActionResult` values, and relies on `portfolio-api` for resource-level authorization.
