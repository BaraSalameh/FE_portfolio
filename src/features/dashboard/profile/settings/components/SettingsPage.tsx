...
export const SettingsPage = () => {
    ...
    return <main className="min-h-svh bg-canvas px-4 py-5 text-ink sm:px-8 sm:py-8"><div className="mx-auto max-w-6xl">
        <div className="flex items-center justify-between gap-4"><Link href={username ? paths.root.dashboard('owner', username).path() : '/'} className="inline-flex min-h-10 items-center gap-2 rounded-full px-3 text-sm font-bold text-ink-muted transition hover:bg-surface hover:text-ink"><ArrowLeft className="size-4" aria-hidden="true" /> Back to dashboard</Link>{user ? <OwnerHeaderActions user={user} unreadMessageCount={unreadContactMessageCount} /> : null}</div>
