...
export const fieldOfStudyListQuery = createAsyncThunk(
    ...
    async ({query, page = 0, pageSize = 5} : FetchAction, thunkAPI)  => {
        try {
            ...
            const response = await dashboardQuery<PaginatedResponse<FieldOfStudyFormData>>({
                method: 'GET',
                url: `/Owner/LKP_FieldOfStudyList?Search=${query}&PageNumber=${page}&PageSize=${pageSize}`,
